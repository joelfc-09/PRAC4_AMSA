...
then
	...
	echo "S'han obtingut els pids"
	rm -rf MEM.user
	touch MEM.user
	cat pids.txt | awk '{system("./PRAC4_2.sh "$1" >> MEM.user")}'
	cat MEM.user | awk -F , 'BEGIN {max_mv=0; max_mr=0; max_tu=0; max_ts=0; ps_mv=0; ps_mr=0; ps_tu=0; ps_ts=0; tot_mv=0; tot_mr=0; tot_tr=0; tot_ts=0}/a*/{tot_mv=tot_mv+$2;tot_mr=tot_mr+$3;tot_tu=tot_tu+$4;tot_ts=tot_ts+$5; 
    ...
    if [ -f /proc/$1/status ]
    then
        cat /proc/$1/status | awk 'BEGIN {ORS = ""; mv = ""; mr = ""}/^VmSize/ { mv = $2 }/^VmRSS/ { mr = $2 }END { print mv ", " mr ", "}'
        cat /proc/$1/stat | awk 'BEGIN {tu = ""; ts = ""}/a*/ {tu = $14; tr = $15}END { print tu ", " tr}'
    fi
fi
